...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1) Validate credentials against DB (single source of truth)
    const { data: player, error: playerError } = await admin
      .from("player")
      .select("pl_id, pl_email, pl_password_site")
      .ilike("pl_email", trimmedEmail)
      .maybeSingle();
    ...
    // NOTE: This assumes pl_password_site is stored as plaintext.
    // If you switch to hashing later, replace this with bcrypt verify.
    if ((player.pl_password_site ?? "") !== rawPassword) {
      return new Response(JSON.stringify({ error: "Invalid login credentials" }), {
        status: 401,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
