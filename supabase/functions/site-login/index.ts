...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Verify password against the bcrypt hash generated by pgcrypto in the database
    const storedHash = player.pl_password_site ?? "";
    const isValid = storedHash.startsWith("$2a$") || storedHash.startsWith("$2b$") 
      ? await bcrypt.compare(rawPassword, storedHash)
      : storedHash === rawPassword; // Fallback for old plaintext passwords if migration wasn't run
